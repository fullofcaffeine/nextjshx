...
`npm run test:adapter-plan` produces the schema-v2 plan from real typed Haxe
...
and then through native Fast Refresh. It also edits a stylesheet requested by
the Haxe root layout and requires the browser's computed style to change and
recover through Next's own development pipeline—there is no CSS copy or second
watcher. A deliberate
