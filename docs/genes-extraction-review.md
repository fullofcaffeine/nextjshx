...
| Exact generic boundary preservation | `nextjs.codec.Decode.accept(...)` must retain a closed enum-abstract result after Haxe erases its String backing representation | Compile-time-only type witnesses for marked generic externs and directly emitted generic Haxe callables, plus safe undefinable presence proofs that erase from classic JavaScript | Compose the generic proof inside a semantic decoder and retain Next-specific form/action diagnostics and runtime evidence | Available in the current genes-ts v1.50.0 pin. PRs [#86](https://github.com/fullofcaffeine/genes-ts/pull/86), [#88](https://github.com/fullofcaffeine/genes-ts/pull/88), and [#90](https://github.com/fullofcaffeine/genes-ts/pull/90) first released this support. The Todo decoder emits the exact closed TypeScript union without an assertion. |
| Genuine module-level function lowering | Page/layout owners that are only static namespaces should match ordinary JS/TS module functions | Direct TS/JS functions, module-local same-name ESM exports/imports, declarations, DCE, maps, and omission of Haxe's synthetic `_Fields_` owner when no residual field needs it | Attach App Router ownership to `render`, validate route/named exports, derive the Genes marker, and emit canonical Next adapters | Upstream `genes-lr1h`, [genes-ts PR #106](https://github.com/fullofcaffeine/genes-ts/pull/106); consumed by `nxhx-xac` |
| Genuine module-level immutable values | Static route metadata should be an ordinary typed `const`, not a reason to restore an all-static class | Direct typed TS/JS `const` bindings, collision-safe cross-module imports, declarations, DCE neutrality, source maps, initialization safeguards, and shared synthetic-owner omission for direct functions/values | Validate Next's exact `Metadata` contract, derive the Genes marker, retain the convention value, alias the implementation import, and publish the canonical adapter export | Upstream `genes-qmqs`, [genes-ts PR #145](https://github.com/fullofcaffeine/genes-ts/pull/145); consumed by `nxhx-f34.9.9` |
...
| Crash-safe generator orchestration | Genes owns updates to compiler output, while `tools/cli/src/generated-output-publisher.ts` owns adapters, validation, last-good recovery, and a separate journal | A separate Genes host tool for exact file updates, crash recovery, HXML/watch handling, serialized builds, an owned Haxe wait server, and a complete development session; see the [decision report](genes-generator-orchestration-decision.md) | App Router ownership transfer, native collisions, Next validation, Next dev-server lifecycle, and `NXHX-*` diagnostics | Genes tooling 0.2.0 is pinned. NextJsHx has the target-free HXML and private compiler-plan bridge. The full `dev` switch waits for the generic session to support normal external Haxe libraries. |
