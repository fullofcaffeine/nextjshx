...
| --- | --- | --- | --- | --- |
| React Hook/component authoring and Flight-shaped value contracts | `nextjshx/client/ReactHooksMacro.hx`, `ReactDiagnosticsMacro.hx`, `ReactAnalyzerFunctionMacro.hx`, `nextjs/client/{State,Optimistic,CachedPromise}.hx`, `nextjs/client/flight/**`, and `nextjs/raw/react/**` | Precise React bindings, HXX prop/children/ref typing, semantic Hook state, dependency and placement checks, analyzer-visible functions, and versioned React transport-value models under `genes.react` | Client/Server Component classification, Next boundary refs, RSC graph rules, and Next build evidence | `nxhx-f34.2.18` |
...
| Crash-safe generator orchestration | genes owns implementation publication while `tools/cli/src/generated-output-publisher.ts` owns adapters, validation, last-good recovery, and a separate journal | Evaluate a reusable host orchestration kernel around isolated generation, exact artifact inventories, rollback, and serialized writers | App Router ownership transfer, native collisions, Next validation, and dev-server lifecycle | `nxhx-f34.2.25` |
