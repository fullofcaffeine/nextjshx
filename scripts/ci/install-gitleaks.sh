...
# The archive checksum is reviewed and pinned in-repository. Downloading a
# checksum beside the archive at runtime would let one compromised source
# replace both files.
...
umask 077
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/nextjshx-gitleaks.XXXXXX")"
