...
function assertSemanticContract(matrix) {
  ...
  const { node, haxe, typescript, genesTooling } = matrix.toolchain;
  ...
  const declaredTooling = packageJson.devDependencies?.[genesTooling.name];
  const lockedTooling = packageLock.packages?.[`node_modules/${genesTooling.name}`];
  if (declaredTooling !== genesTooling.archiveUrl) {
    throw new MatrixFailure("package.json must use the exact Genes tooling archive URL");
  }
  if (
    lockedTooling?.version !== genesTooling.version ||
    lockedTooling?.resolved !== genesTooling.archiveUrl ||
    lockedTooling?.integrity !== genesTooling.npmIntegrity
  ) {
    throw new MatrixFailure("package-lock.json must pin the reviewed Genes tooling archive");
  }
...
function renderCompatibilityDoc(matrix) {
  ...
  const lines = [
    ...
    `| @genes-ts/tooling | ${genesTooling.version} from [GitHub Release](${genesTooling.releaseUrl}) | npm installs the exact archive. It is not published to the npm registry. SHA-256: \`${genesTooling.sha256}\`. |`,
