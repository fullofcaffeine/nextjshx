...
/**
 * Keyboard command surface rendered inside the existing Todo client boundary.
 *
 * cmdk owns accessible dialog/list behavior. Haxe owns the command algebra,
 * URL-backed intent calls, route construction, and exhaustive dispatch.
 */
class TodoCommandCenter {
	public static function render(props:TodoCommandCenterProps):Element {
		final moveItems = [
	...
	static function commandItem(command:TodoCommand, props:TodoCommandCenterProps):Element {
	...
	static function todoCommandItem(todo:Todo, props:TodoCommandCenterProps):Element {
	...
	static function execute(command:TodoCommand, props:TodoCommandCenterProps):Void {
		switch command {
			case FocusCreate:
				props.setOpen(false);
			case FocusSearch:
	...
	static function openTodo(todo:Todo, props:TodoCommandCenterProps):Void {
		props.setOpen(false);
		props.router.push(TodoDetailPage.href({id: todo.id}));
	}

	static function focusTarget(command:TodoCommand):TodoFocusTarget {
	...
	static function value(command:TodoCommand):String {
		return switch command {
	...
	static function code(command:TodoCommand):String {
		return switch command {
			case FocusCreate: "N";
			case FocusSearch: "/";
			case ResetLens: "00";
	...
	static function label(command:TodoCommand):String {
		return switch command {
	...
	static function description(command:TodoCommand):String {
		return switch command {
	...
	static function keywords(command:TodoCommand):Array<String> {
		return switch command {
