...
/** Typed in-memory field journal used to prove static App Router generation. */
class PostCatalog {
	public static function featured():Post {
		return afterTheBurn();
	}

	public static function fieldNotes():Array<Post> {
		return [trailPromise(), snowline()];
	}

	public static function all():Array<Post> {
		return [afterTheBurn(), trailPromise(), snowline()];
	}

	public static function find(slug:PostSlug):Null<Post> {
		for (post in all()) {
			if (post.slug == slug) {
				return post;
			}
	...
	public static function nextAfter(slug:PostSlug):Post {
	...
	static function afterTheBurn():Post {
		return {
			slug: "after-the-burn",
	...
	static function trailPromise():Post {
		return {
