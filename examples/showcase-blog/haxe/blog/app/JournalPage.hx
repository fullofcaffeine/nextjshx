...
/**
 * Composes the root journal page from one typed catalogue.
 *
 * `@:next.page("")` makes this normal module function the source owner of
 * `app/page.tsx`. NextJsHx checks its props and derives `href()` for links back
 * to `/`; Next still performs ordinary Server Component rendering.
 */
@:next.page("")
function render(props:PageProps<NoParams, SearchParams>):Element {
	...
	return <main>
			<header className="journal-header">
				<NextLink className="journal-mark" href={href()}><span>M</span><strong>Moraine</strong><small>Field journal / est. 2019</small></NextLink>
...
/**
 * Projects one closed catalogue entry into a reusable card.
 *
 * Keeping this helper private makes the module's public surface match the
 * generated Next page exports while HXX still checks the card composition.
 */
private function renderPostCard(post:Post):Element {
