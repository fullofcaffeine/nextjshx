...
/**
 * `@:next.page("haxe-lab")` declares the App Router segment on the ordinary
 * module-level `render` function. A class would add no identity or state here,
 * so NextJsHx derives Genes' direct ESM function lowering and owns only the
 * resulting `app/haxe-lab/page.tsx` adapter. Next still discovers and renders
 * that conventional file.
 */
@:next.page("haxe-lab")
/**
 ...
 * boundary. Native React still hydrates both entries in the same module graph;
 * this function introduces no interop runtime.
 */
